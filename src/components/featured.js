...
const Featured = ({type, setGenre, listdata, film, config}) => {
    ...
    useEffect(()=> {
        const getRandom = async () => {
            try {
                const {data} = await axios.get(API_URL + 'movie/random?type='+ type, { headers: authHeader() })
     ...
     (
            ...
            <div className="info">
                ...
                <div className="more">
                    ...
                    <Link to={`/content/${random?.id}`} style={{textDecoration: 'none', color: 'white',display: 'flex', alignItems: 'center'}}>
