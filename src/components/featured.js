    ...
    >
      ...
      {!isHovered ? (
        <div
          ...
          style={{
            backgroundImage: `linear-gradient(to bottom, transparent, #000000), url('${random?.thumbnail}')`,
            objectFit: 'cover',
          }}
        ></div>
      ) : (
      ...
      <div className='info'>
        ...
        <span className='rated'>
          Drama<b>+{random?.age_rating}</b>
        </span>
        {isClicked ? (
          <div className='more'>
            <span>Directed By {random?.director}</span>
            <span>{random?.year}</span>
            <span>{time_convert(random?.episodes[0]?.duration)}</span>
            <Link
        ...
        <div className='buttons'>
          ...
          >
            <InfoOutlined
              style={{ width: '0.5em', height: '0.7em', color: 'black' }}
            />
            <span className='actions' style={{ marginLeft: '5px' }}>
              Info
            </span>
          </button>
        </div>
